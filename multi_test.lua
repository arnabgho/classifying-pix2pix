...
opt = {
    ...
    ngen=3,                   -- The number of generators to be used
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    for gen_id=1,opt.ngen do
        ...
        if opt.preprocess == 'colorization' then
           local output_AB = G['netG'..gen_id]:forward(input):float()
           ...
           input = util.deprocessL_batch(input_L)
            ...
            output = util.deprocess_batch(G['netG'..gen_id]:forward(input))
        ...
        local image_dir = paths.concat(opt.results_dir, opt.netG_name .. '_' .. opt.phase, 'images' .. gen_id )
