                ...
                for i5 in 1,ngen do
                    ...
                    if opt.preprocess == 'colorization' then 
                        for i2=1, dfake_B:size(1) do
                            if image_out==nil then image_out = torch.cat(util.deprocessL(real_A[i2]:float()),util.deprocessLAB(real_A[i2]:float(), dfake_B[i2]:float()),3)/255.0
                            else image_out = torch.cat(image_out, torch.cat(util.deprocessL(real_A[i2]:float()),util.deprocessLAB(real_A[i2]:float(), dfake_B[i2]:float()),3)/255.0, 2) end
                            image.save(paths.concat(opt.checkpoints_dir,  opt.name , counter .. i5 .. '_train_res.png'), image_out)
                        end
                    else
                        for i2=1, dfake_B:size(1) do
                            if image_out==nil then image_out = torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(dfake_B[i2]:float()),3)
                            else image_out = torch.cat(image_out, torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(dfake_B[i2]:float()),3), 2) end
                            image.save(paths.concat(opt.checkpoints_dir,  opt.name , counter .. i5 .. '_train_res.png'), image_out)
                        end
                    end
                end
